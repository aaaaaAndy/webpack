...
/**
 * Compiler和Compilation是webpack的两大引擎，此处是Compiler引擎；
 * 记录了完整的webpack环境信息，可以从compiler上读取到webpack config信息等；
 * 在webpack从启动到结束，只会实例化一次
 */
class Compiler extends Tapable {
	constructor(context) {
		super();
		this.hooks = {
			/** @type {SyncBailHook<Compilation>} 是否应该执行一次emit */
			shouldEmit: new SyncBailHook(["compilation"]),
			/** @type {AsyncSeriesHook<Stats>} 一次编译完成后执行 */
			done: new AsyncSeriesHook(["stats"]),
			/** @type {AsyncSeriesHook<>} */
			additionalPass: new AsyncSeriesHook([]),
			/** @type {AsyncSeriesHook<Compiler>} */
			beforeRun: new AsyncSeriesHook(["compiler"]),
			/** @type {AsyncSeriesHook<Compiler>} 在编译器开始读取记录前执行 */
			run: new AsyncSeriesHook(["compiler"]),
			/** @type {AsyncSeriesHook<Compilation>} 在生成文件到output目录之前执行 */
			emit: new AsyncSeriesHook(["compilation"]),
			/** @type {AsyncSeriesHook<string, Buffer>} 在生成文件到output目录之后执行 */
			...
			/** @type {SyncHook<Compilation, CompilationParams>} 在一次compilation创建之后开始执行 */
			...
			/** @type {SyncHook<CompilationParams>} 在一个新的compilation创建之前执行 */
			compile: new SyncHook(["params"]),
			/** @type {AsyncParallelHook<Compilation>} 完成一次编译之前执行 */
	...
	/**
	 * 监听文件变化，实例化一个Watching对象
	 * @param watchOptions
	 * @param handler
	 * @returns {Watching|*}
	 */
	watch(watchOptions, handler) {
		// 如果运行在watch模式，则执行callback方法
	...
	run(callback) {
		...
		const onCompiled = (err, compilation) => {
			...
			// 判断是否需要执行emit，如果不需要就会return，到此为止
